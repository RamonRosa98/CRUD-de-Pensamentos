...
async function manipularSubmissaoFormulario(event) {
    ...
    if(!validarConteudo(conteudoSemEspaco)){
        alert("É permitido apenas a inclusão apenas de letras e espaços com no mínimo 10 caracteres");
        return;
    };

    if(!validarAutoria(autoriaSemEspaco)) {
        alert("É permito apenas a inclusão de letras e espaços com no mínimo 3 caracteres e no máximo 15");
        return;
    };
